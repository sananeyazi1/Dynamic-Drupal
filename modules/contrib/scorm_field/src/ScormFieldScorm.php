...
class ScormFieldScorm {
  ...
  public function unzipPackage(File $file) {
    ...
    if ($result === TRUE) {
      $extract_dir = 'public://scorm_field_extracted/scorm_' . $file->id();
      ...
      \Drupal::logger('scorm_field')->error("An error occured when unziping the SCORM package data. Error: !error", ['!error' => $error]);
    }
  ...
  public function scormSave($scorm) {   
    ...
    if (!empty($scorm->id)) {
      return $connection->update('scorm_field_scorm_packages')
        ->fields((array) $scorm)
        ->condition('id', $scorm->id)
        ->execute();
    }
    else {
      $id = $connection->insert('scorm_field_scorm_packages')
        ->fields((array) $scorm)
        ->execute();
  ...
  public function scormScoSave($sco) {
    ...
    if (!empty($sco->id)) {
      $res = $connection->update('scorm_field_scorm_package_scos')
        ->fields((array) $sco)
        ->condition('id', $sco->id)
        ->execute();
    }
    else {
      $id = $connection->insert('scorm_field_scorm_package_scos')
        ->fields((array) $sco)
        ->execute();
    ...
    if ($res && !empty($attributes)) {
      ...
      $connection->delete('scorm_field_scorm_package_sco_attributes')
      ...
      foreach ($attributes as $key => $value) {
        ...
        $connection->insert('scorm_field_scorm_package_sco_attributes')
          ->fields([
            'sco_id' => $sco->id,
            'attribute' => $key,
            'value' => $value,
  ...
  public function scormLoadByFileEntity(File $file) {
    $connection = $this->database;
    return $connection->select('scorm_field_scorm_packages', 'o')
      ->fields('o', [])
  ...
  public function scormLoadSco($sco_id) {
    $connection = $this->database;
    $sco = $connection->select('scorm_field_scorm_package_scos', 'o')
      ->fields('o', [])
      ->condition('id', $sco_id)
      ->execute()
      ->fetchObject();
  ...
  private function scormLoadScormAttributes($sco_id) {
    ...
    $result = $connection->select('scorm_field_scorm_package_sco_attributes', 'o')
