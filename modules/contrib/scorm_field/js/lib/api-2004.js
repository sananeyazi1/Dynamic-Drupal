 ...
 *
 * This is the Scorm Field SCORM UI implementation of the SCORM API
 * object.
 */
...
;(function($, Drupal, window, undefined) {
   ...
   * @constructor
   */
  var ScormFieldScorm2004API = function() {
    this.version = '1.0.0';
    this.error = '0';
   ...
   */
  ScormFieldScorm2004API.VALUE_NOT_AVAILABLE = 'VALUE_NOT_AVAILABLE';

  /**
   * @const Requested CMI value is invalid.
   */
   ScormFieldScorm2004API.CMI_NOT_VALID = 'CMI_NOT_VALID';

  /**
   * @const Requested CMI value is not yet implemented by Scorm Field Scorm.
   */
   ScormFieldScorm2004API.CMI_NOT_IMPLEMENTED = 'CMI_NOT_IMPLEMENTED';

  /**
   * @const Requested CMI value is write-only.
   */
   ScormFieldScorm2004API.VALUE_WRITE_ONLY = 'VALUE_WRITE_ONLY';

  /**
   * @const Requested CMI value is read-only.
   */
   ScormFieldScorm2004API.VALUE_READ_ONLY = 'VALUE_READ_ONLY';

  /**
   * @const Requested CMI child value does not exist.
   */
   ScormFieldScorm2004API.CHILD_DOES_NOT_EXIST = 'CHILD_DOES_NOT_EXIST';
   ...
   */
   ScormFieldScorm2004API.prototype.Initialize = function(value) {
   ...
   *        The SCO expects boolean values to be returned as strings.
   */
   ScormFieldScorm2004API.prototype.Terminate = function(value) {
    // The value MUST be an empty string (per SCORM.2004.3ED.ConfReq.v1.0).
      ...
      if (false) {
        ...
        if (this.adl_nav_request != "_none_") {
          if (this.adl_nav_request.indexOf('choice') !== -1) {
            ...
            var scoId = Drupal.settings.scormFieldScormUIPlayer.scoIdentifiers[scoIdetifier];
            var $el = $(document),
            ...
            // Load the SCO.
            iframe.src = Drupal.settings.basePath + ScormFieldScormUIPlayer.PATH_PATTERN.replace('%sco_id', scoId);
   ...
   * @returns {String}
   */
   ScormFieldScorm2004API.prototype.GetValue = function(cmiElement) {
    ...
    try {
      if (/^cmi\./.test(cmiElement)) {
        ...
        // and return ''.
        if (result === ScormFieldScorm2004API.VALUE_NOT_AVAILABLE) {
          this.error = '403';
          return '';
        }
        // If the value does not exist, set the error to 401
        // and return ''.
        else if (result === ScormFieldScorm2004API.CMI_NOT_VALID) {
          this.error = '401';
          return '';
        }
        // If the value is supposed to be a child value, but the parent
        // doesn't have it, set the error to 301 and return ''.
        else if (result === ScormFieldScorm2004API.CHILD_DOES_NOT_EXIST) {
        ...
        // return ''.
        else if (result === ScormFieldScorm2004API.VALUE_WRITE_ONLY) {
          this.error = '405';
          return '';
        }
   ...
   * @return {String}
   */
   ScormFieldScorm2004API.prototype.SetValue = function(cmiElement, value) {
    // Cannot get a value if not initialized.
    // Set the error to 122 end return ''.
    if (!this.isInitialized) {
      this.error = '132';
   ...
   *        The SCO expects boolean values to be returned as strings.
   */
   ScormFieldScorm2004API.prototype.Commit = function(value) {
    // The value MUST be an empty string (per SCORM.2004.3ED.ConfReq.v1.0).
    // If it's not empty, don't bother terminating the package.
    if (value !== '') {
      this.error =  '201';
   ...
   * @returns {String}
   */
   ScormFieldScorm2004API.prototype.GetLastError = function() {
    return this.error;
  }
   ...
   * @return {String}
   */
   ScormFieldScorm2004API.prototype.GetErrorString = function(cmiErrorCode) {
    // @todo
    return '';
  }
   ...
   * @return {String}
   */
   ScormFieldScorm2004API.prototype.GetDiagnostic = function(cmiErrorCode) {
    // @todo
    return '';
  }
   ...
   * the ScormFieldScorm2004API object directly.
   */

  /**
   *
   */
   ScormFieldScorm2004API.prototype.normalizeCMIPath = function(cmiPath) {
   ...
   */
   ScormFieldScorm2004API.prototype.bind = function(event, callback) {
   ...
   * @param {String} event
   */
   ScormFieldScorm2004API.prototype.trigger = function() {
   ...
   */
   ScormFieldScorm2004API.prototype.registerCMIPaths = function(cmiPaths) {
   ...
   * @param {Object} data
   */
   ScormFieldScorm2004API.prototype.registerCMIData = function(cmiPath, data) {
    this._setCMIData(cmiPath, data, true);
  }

  ScormFieldScorm2004API.prototype.registerSuspendItems = function(suspendItems) {
   ...
   * or extending the ScormFieldScorm2004API object. Method signatures can change.
   */

  /**
   * Initialize the communication between the SCORM package and Drupal.
   *
   * @return {Boolean}
   */
   ScormFieldScorm2004API.prototype._initCommunication = function() {
    ...
    // and Drupal must always be considered "active", and can never "fail".
   ...
   * @returns {String}
   */
   ScormFieldScorm2004API.prototype._getCMIData = function(cmiPath, skipValidation) {
    if (!skipValidation) {
      // Special test values.
      if (cmiPath === 'cmi.__value__') {
        ...
        return ScormFieldScorm2004API.VALUE_WRITE_ONLY;
      }
      else if (cmiPath === 'cmi.__unimplemented__') {
        return ScormFieldScorm2004API.CMI_NOT_IMPLEMENTED;
      }
      else if (cmiPath === 'cmi.__unknown__') {
        return ScormFieldScorm2004API.CMI_NOT_VALID;
      }
    ...
    if (data !== null) {
      ...
      if (checkChildren) {
        return ScormFieldScorm2004API.CHILD_DOES_NOT_EXIST;
      }
      // Else, return VALUE_NOT_AVAILABLE.
      else {
        return ScormFieldScorm2004API.VALUE_NOT_AVAILABLE;
      }
    }
  }
   ...
   * @returns {String}
   */
   ScormFieldScorm2004API.prototype._setCMIData = function(cmiPath, value, skipValidation) {
    if (!skipValidation) {
      // Check if the CMI path is valid. If not, return CMI_NOT_VALID.
      if (!this._validCMIDataPath(cmiPath)) {
        return ScormFieldScorm2004API.CMI_NOT_VALID;
      ...
      // Check if the CMI path is read-only. If so, return VALUE_READ_ONLY.
        ...
        return ScormFieldScorm2004API.VALUE_READ_ONLY;
      }
    }
   ...
   * @returns {Boolean}
   */
   ScormFieldScorm2004API.prototype._validCMIDataPath = function(cmiPath) {
    // Normalize the path.
   ...
   * @returns {Boolean}
   */
   ScormFieldScorm2004API.prototype._writeOnlyCMIDataPath = function(cmiPath) {
    // Normalize the path.
    var normalizedPath = this.normalizeCMIPath(cmiPath);
   ...
   * @returns {Boolean}
   */
  ScormFieldScorm2004API.prototype._readOnlyCMIDataPath = function(cmiPath) {
  ...
  /**
   * Check if the given CMI path is implemented by Scorm field.
   *
   * @param {String} cmiPath
   *
   * @returns {Boolean}
   */
   ScormFieldScorm2004API.prototype._implementedCMIDataPath = function(cmiPath) {
  ...
  // Export.
  window.ScormFieldScorm2004API = ScormFieldScorm2004API;
